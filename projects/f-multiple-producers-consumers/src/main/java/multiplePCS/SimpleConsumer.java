...
public class SimpleConsumer implements Runnable {
			...
			while (true) { 
				...
				for (ConsumerRecord<String, String> record : records) 
				{
					...
					System.out.println(props.getProperty("client.id")+" "
							+ "messaggio ricevuto: "+String.format("valore: %s, Topic: %s, Partition: %d", record.value(), record.topic(), record.partition()));
				}
			}
		}catch (Exception e){
	...
	public String toString() {
		return "consumer "+props.getProperty("client.id")+" appartentente al gruppo: "+ 
				props.getProperty("group.id")+ " iscritto al topic: " + this.topic;
	}
